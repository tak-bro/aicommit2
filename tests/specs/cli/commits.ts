...
export default testSuite(({ describe }) => {
    if (process.platform === 'win32') {
        ...
        console.warn('Skipping tests on Windows because Node.js spawn cannot open TTYs');
        return;
    }
    ...
    describe('Commits', async ({ test, describe }) => {
        test('Excludes files', async () => {
            ...
            const { stdout, exitCode } = await aicommit2(['--exclude', 'data.json'], { reject: false });
        ...
        test('Accepts --all flag, staging all changes before commit', async () => {
            ...
            const committing = aicommit2(['--all']);
            committing.stdout!.on('data', (buffer: Buffer) => {
                const stdout = buffer.toString();
                if (stdout.match('└')) {
                    committing.stdin!.write('y');
            ...
            expect(statusAfter.stdout).toBe('?? .aicommit2');
        ...
        test('Accepts --generate flag, overriding config', async ({ onTestFail }) => {
            ...
            const committing = aicommit2(['--generate', '2']);
        ...
        describe('commit types', ({ test }) => {
            ...
            test('Accepts --type flag, overriding config', async () => {
                ...
                // Generate flag should override generate config
                const committing = aicommit2(['--type', 'conventional']);
            ...
            test('Accepts empty --type flag', async () => {
                ...
                const committing = aicommit2(['--type', '']);
